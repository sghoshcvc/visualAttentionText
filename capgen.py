...
#from get_lang_prob_trie import get_lang_prob
#
...
#            'synthText': (synthText.load_data, synthText.prepare_data),
#            'synthTextBigDB': (synthText_bigDB.load_cap, synthText_bigDB.prepare_data,synthText_bigDB.load_data),
#            'cocoText': (cocoText.load_cap, cocoText.prepare_data,cocoText.load_data)}
    ...
    for ii in xrange(maxlen):
        ...
        if stochastic:
            ...
            if trie != None:
                ...
                cand_scores = hyp_scores[:,None] - ((1-alpha)*numpy.log(next_p)) -(alpha * numpy.log(next_langP))  # add language probability  
...
def pred_probs(f_log_probs, options, worddict, prepare_data, data, iterator, verbose=False,startIndx=0):
    ...
    for _, valid_index in iterator:
        x, mask, ctx = prepare_data([data[0][t] for t in valid_index],
                                     ...
                                     worddict,
                                     maxlen=None,
                                     n_words=options['n_words'],startIndx=startIndx)
